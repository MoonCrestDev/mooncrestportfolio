...
export const Card: React.FC<CardProps> = ({
    ...
    return (
        <div
            ...
            className={`flex flex-col h-full rounded-2xl p-5 m-2 md:m-4 space-y-2 items-center justify-evenly bg-[--bg] ${fade ? "opacity-100" : "opacity-0 pointer-events-none"} transition-all ease-in duration-200 ${show ? "" : "absolute"}`}>
            ...
            <div className="flex flex-row space-x-2 items-center">
                ...
                <h1
                    ...
                    className={`font-bold text-2xl text-[--on_bg]`}>
                    {title.title}
                </h1>
            </div>
            ...
            <div className={"flex flex-col xl:flex-row space-x-2 items-start justify-center"}>
                ...
                {presentingImage && (
                    <img className={`transition-all duration-500 ${!isExpanded ? "w-fit" : "pointer-events-none md:pointer-events-auto w-0 md:w-fit"}`} src={presentingImage} alt="prsenting_image"/>
                )}
            </div>
